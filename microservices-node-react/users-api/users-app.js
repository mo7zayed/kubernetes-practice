...
const config = {
  authServiceUrl: process.env.AUTH_ADDRESS || process.env.AUTH_SERVICE_SERVICE_HOST
};

app.use(bodyParser.json());

app.get('/', (req, res) => {
  res.status(200).json({ message: 'Hey there :), You are in users api' });
})
...
app.post('/signup', async (req, res) => {
  ...
  try {
    const hashedPW = await axios.get(`http://${config.authServiceUrl}/hashed-password/` + password);
...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get(
    `http://${config.authServiceUrl}/token/${hashedPassword}/${password}`
  );

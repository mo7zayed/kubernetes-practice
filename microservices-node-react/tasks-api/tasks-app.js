...
const config = {
  ...
  tasksFolder: process.env.TASKS_FOLDER,
};

const tasksDir = path.join(__dirname, config.tasksFolder);
...
const extractAndVerifyToken = async (headers) => {
  ...
  const response = await axios.get(`http://${config.authServiceUrl}/verify-token/${token}`);
